...
users/{uid}/levels/level1                (document)
  ...
  timesCompleted: number                 total successful completions (persists across attempts)
  lastAttemptAt: timestamp                set each time a fresh attempt is started
  lastCompletedAt: timestamp              set each time the level is completed
...
- The 6 questions are treated as a single fixed pool for "level1". The main
  menu now shows per-level stats (times completed, last completed date,
  last attempt date) and lets the user start fresh or resume a paused
  (in-progress) attempt.
